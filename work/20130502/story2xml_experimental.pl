...
sub test_args {
    ...
    return ($opts{"d"}, $default_out, $the_input_file, $story_file_out);
}
...
sub parse_story() {
	...
	if ($_[$i] =~ s/^[Ss]cenario:\s*(.*)/<p>$1<\/p>/) {
	    print "$_[$i]\n";
	    # an extra while-loop may be needed for the "Given" group too.
