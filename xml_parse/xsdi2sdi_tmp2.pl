...
#		    }
			...
			if (!$open_tag) {
			    ...
			    $$chapter_category_collect{"chl".$chl_depth}{$_}++ foreach @category_filters;
			    #print STDERR "category_filters: @category_filters\n";
			    #print STDERR  "category hash:\n";
			    #print STDERR ("$_ => $$chapter_category_collect{"chl".$chl_depth}{$_}\n") foreach (sort keys $$chapter_category_collect{"chl".$chl_depth});
			}
		    }
		...
		unless ($open_tag eq "") {
		    ...
		    if ($open_tag =~ /chl\d/ and !$inside_category_filters) {
			$out_lines[$saved_line_numbers[-1]] =~ s/<title>\s*/<title>/;
			...
			#print STDERR "filter hash: ",$$chapter_category_collect{"chl".$chl_depth},"\n";
		    }
		    ...
		    if ($open_tag =~ /chl\d/) {
			print STDERR "opening line: $out_lines[$saved_line_numbers[-1]]";
			if ($$chapter_category_collect{"chl".$chl_depth} and !$inside_category_filters) {
			    #print STDERR ("$_ => $category_filter_hash{$_}\n") foreach (sort keys %category_filter_hash);
			    print STDERR "<!-- |";
			    print STDERR " $_" foreach (sort keys $$chapter_category_collect{"chl".$chl_depth});
			    print STDERR "-->";
			    print STDERR "\n";
			    print STDERR "chapter depth: $chl_depth\n";
			    print STDERR "Chapter depth: ",%{$$chapter_category_collect{"chl".$chl_depth}},"\n";
			    unless ($chl_depth == 1) {
				foreach (keys $$chapter_category_collect{"chl".$chl_depth}) {
				    my $temp_key = "chl".($chl_depth-1);
				    print STDERR "temp_key: $temp_key\n";
				    print STDERR "inside: ",$$chapter_category_collect{$temp_key}{$_}++,"\n";
				    #$$chapter_category_collect{"chl".$chl_depth-1}{$_}++;
				}
			    }
			    $$chapter_category_collect{"chl".$chl_depth} = ();
			}
			print STDERR "current line: $out_lines[$line_number-1]";
		    }
		    if ($open_tag =~ /chl\d/) {
			$chl_depth--;
			print STDERR "Saved line numbers: @saved_line_numbers\n";
			my $popped_number = pop(@saved_line_numbers);
			print STDERR "popped_number: $popped_number\n";
			#print STDERR "line of popped number: $out_lines[$popped_number]";
			print STDERR "line still stored: $out_lines[$saved_line_numbers[-1]]" if @saved_line_numbers;
			print STDERR "chl_depth: $chl_depth\n" if $chl_depth;
			print STDERR "Chapter depth: ",%$chapter_category_collect,"\n" if $chl_depth;
		    }
		    $open_tag = "";
		    print STDERR "\\n\n" if ($DEBUG == 9);
		}
		...
		if (ref($$xml_tree[$_+1]) =~ /ARRAY/) {
			...
			$out_lines[$line_number] .= $open_tag;
			push @saved_line_numbers,$line_number;
			$is_tag_printed = 1;
			print STDERR "<<<chl>>>\n" if ($DEBUG == 9);
