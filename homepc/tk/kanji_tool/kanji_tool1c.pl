...
my $fn_kanji = "kanji_data.dat";
my $fn_words = "word_data.dat";
my %kanji_data = %{retrieve($fn_kanji)};
my %word_data = %{retrieve($fn_words)};;
...
sub fill_notebook {
    my $raised_tab = 0;
    foreach (sort keys %tab_contents) {
	$the_tabs{$_} = $kanji_notes->add($_, -label => $_);
	$tab_txt_boxes{$_} = $the_tabs{$_}->Scrolled("ROText",-scrollbars => 'oe');
	$tab_txt_boxes{$_}->configure(-font => $example_font, -wrap => 'word', width => '80', -height => '10', -tabs => [qw(0.5i left)]);
	$tab_txt_boxes{$_}->pack(-expand => 1, -fill => 'both');
	unless ($tab_contents{$_} eq 'inactive') {
	    $tab_txt_boxes{$_}->insert('end',$tab_contents{$_});
	    unless ($raised_tab) {
		$kanji_notes->raise($_) unless ($kanji_notes->raised());
		$raised_tab = 1;
	    }
	} else {
	    $kanji_notes->pageconfigure($_, -state => 'disabled');
	}
    }
}
...
#	print "$_\n";
#	$tab_txt_boxes{$_}->delete("1.0",'end');
	$tab_txt_boxes{$_}->destroy;
	$kanji_notes->delete($_);
	delete $tab_txt_boxes{$_};
	delete $the_tabs{$_};
	delete $tab_contents{$_};
    }
}

sub draw_kanji {
    &empty_notebook if (%the_tabs);
    my $kanji_to_draw = $kanji_var;
    my $collect_string = "";
    $kanji_lbl->configure(
    ...
    foreach my $kanji_info (@kanji_tab_labels) {
	$collect_string = (@{$kanji_data{$kanji_var}{$display_select}{$kanji_info}}) ? "$kanji_info：\n" : 'inactive';
	foreach (@{$kanji_data{$kanji_var}{$display_select}{$kanji_info}}) {
	    $collect_string .= "$_\n";
	 }
	$tab_contents{$kanji_info} = $collect_string;
	$collect_string = "";
    }
...
sub print_kana {
	...
	foreach $the_word (@example_words) {
		...
		foreach (sort keys %{$word_data{$the_word}{平仮名}}) {
		    $examples_to_print .= "（@{${$word_data{$the_word}{平仮名}}{$_}}）：";
		}
