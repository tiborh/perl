...
@return_array = $ARGV[0] ? &draw_kana_table(\@片仮名五十音,\@平仮名五十音,\%仮名_btns,\%仮名_var) : &draw_kana_table(\@平仮名五十音,\@片仮名五十音,\%仮名_btns,\%仮名_var);

%仮名_btns = %{$return_array[0]};
%仮名_var = %{$return_array[1]};
...
# Kana Buttons
	...
	-command => sub {
		...
		if ($仮名_var{$_}) {
		    ...
		    $仮名_btns{$仮名一[$gridx][$gridy]}->configure(-text => $仮名一[$gridx][$gridy]);
