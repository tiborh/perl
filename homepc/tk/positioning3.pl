...
sub come_window {
    ...
    $balloon->attach($south_btn, 
		     -balloonmsg => "みなみ",
		     -statusmsg => "-side => \"bottom\", -expand =>1, -fill=>\"x\"",
		     -balloonposition => 'mouse',
	);
	...
	)->pack(-side => "left", -expand => 1, -fill => "y", -after => $south_btn);
    $balloon->attach($west_btn, 
		     -balloonmsg => "にし",
		     -statusmsg => "-side=>\"left\", -expand=>1, -fill=>\"y\"",
		     -balloonposition => 'mouse',
	);
	...
	)->pack(-side => "right", -expand => 1, -fill => "y", -after => $north_btn);
    $balloon->attach($east_btn, 
		     -balloonmsg => "ひがし",
		     -statusmsg => "-side=>\"right\", -expand=>1, -fill=>\"y\"",
		     -balloonposition => 'mouse',
	);
    MainLoop;
}
